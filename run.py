...
def tokenize_and_mask(text, span_length, pct, ceil_pct=False):
    """
    Tokenizes the input text and masks certain spans of the text with a special mask token. 
    
    text (str): input text
    span_length (int): The length of each masked span.
    pct (float): The percentage of the text to be masked.   

    Returns: The masked text with the mask tokens replaced with special tokens.
    """
    tokens = text.split(' ')
    mask_string = '<<<mask>>>'
...
def count_masks(texts):
    """
    Counts the number of mask tokens in a list of texts.
    
    texts (list): input texts on which masked tokens need to be counted.

    Returns: Counts of masked tokens in input texts.
    """
...
def replace_masks(texts):
    """
    Replaces the mask tokens in a list of texts with new text generated by the mask model. 
    
    texts (list): input texts in which masked tokens need to be replaced.
...
def extract_fills(texts):
    """
    Extracts the filled-in text from a list of texts where the mask tokens have been replaced with new text.

    texts (list): Input texts from which filled-in texts need to be extracted. 
    
    Returns: Extracted filled-in text from the input texts.
    """
...
def apply_extracted_fills(masked_texts, extracted_fills):
    """
    Replaces the mask tokens in a list of masked texts with the filled-in text extracted from the previous step. 

    masked_texts (list): list of masked texts.
    extracted_fills (list): list containing filled-in text extracted from original texts.

    Returns: the new texts.
    """
...
def perturb_texts_(texts, span_length, pct, ceil_pct=False):
    """
    Masks the input texts, generates new text to fill in the masks, and replaces the mask tokens with the generated text. 

    texts (list): A list of texts to be perturbed.
    span_length (int): The length of each masked span.
    pct (float): The percentage of the text to be masked.
    ceil_pct (bool): Whether to round up the calculated number of masked spans to an integer.

    Returns: the list of perturbated texts.
    """
    if not args.random_fills:
...
def perturb_texts(texts, span_length, pct, ceil_pct=False):
    """
    This function applies perturbations to the given input texts.
    
    texts (list): input texts to be perturbed
    span_length (int): the length of the perturbed span in each text
    pct (float): the percentage of text to be perturbed in each text
    ceil_pct (bool): whether to round up the percentage of text to be perturbed

    Returns: Perturbations of the input list of texts.
    """
...
def drop_last_word(text):
    """
    This function drops the last word from a given text.

    text (str): the input text from which the last word is to be dropped 

    Returns: Input text after dropping the last word.
    """
...
def _openai_sample(p):
    """
    This function samples from the OpenAI model using the given prompt. 
    
    p (str): the prompt to be used for sampling from the OpenAI model

    Returns: Combined form of prompt and the samples from the OpenAI model.
    """
...
def sample_from_model(texts, min_words=55, prompt_tokens=30):
    """
    Function samples from the base model using only the first 30 tokens in each example as context.
    
    texts (list): input texts to be used as context for the base model
    min_words (int): the minimum number of words that the generated text should have
    prompt_tokens (int): the number of tokens to be used as context for the base model

    Returns: Generated samples from the base model. 
    """
...
def get_likelihood(logits, labels):
    """
    This function calculates the log likelihood of the given logits and labels. 
    
    logits (tensor): Shape (1, seq_length, vocab_size) representing the logits returned by the model
    labels (tensor): Shape (1, seq_length) representing the labels for the input text

    Returns: Mean of the log likelihood between logits and labels.
    """
...
def get_ll(text):
    """
    Calculates the log-likelihood of a given text under a base model.    
    
    text (string): Input text of whose log likelihood is to be calculated.
    
    Returns: Log likelihood of input text.
    """
    if args.openai_model:        
...
def get_lls(texts):
    """
    Calculates the log-likelihood of a list of texts under a base model. 
    
    texts (list): inputs texts whose log likelihood is to be calculated.
    
    Returns: Log likelihood of list of input texts. 
    """
...
def get_rank(text, log=False):
    """
    Calculates the average rank of each observed token sorted by model likelihood.
    
    text (string): The input text for which to compute the rank.
    log (bool, optional): Whether to compute the log rank. Defaults to False.
    
    Returns: The rank of the input text. 
    """
...
def get_entropy(text):
    """
    Calculates the average entropy of each token in a given text.
    
    text (string): The input text for which to compute the entropy.
    
    Returns: The entropy value from the input text. 
    """
...
def get_roc_metrics(real_preds, sample_preds):
    """
    Calculates receiver operating characteristic (ROC) curve metrics
    
    real_preds (list of floats): A list of real predictions (i.e., ground truth labels) for which to compute ROC metrics.
    sample_preds (list of floats): A list of sample predictions (i.e., predicted probabilities) for which to compute ROC metrics.

    Returns: False Positive Rate, True Positive Rate and AUC from real and sample predictions.
    """
...
def get_precision_recall_metrics(real_preds, sample_preds):
    ...
    real_preds (list of floats): A list of real predictions (i.e., ground truth labels) for which to compute precision-recall metrics.
    sample_preds (list of floats): A list of sample predictions (i.e., predicted probabilities) for which to compute precision-recall metrics.

    Returns: Precision, Recall and AUC values from real and sample predictions.
    """
...
def save_roc_curves(experiments):
    """
    Saves ROC curves for a list of experiments.
    experiments (list): list containing the metrics values for several experiments on the input texts.
    """
    # first, clear plt
    plt.clf()
...
def save_ll_histograms(experiments):
    """
    Saves histograms of log-likelihoods for a list of experiments. 
    experiments (list): list containing the metrics values for several experiments on the input texts.
    """
    # first, clear plt
    plt.clf()
...
def get_perturbation_results(span_length=10, n_perturbations=1, n_samples=500):
    """
    Produces results from the source model on the perturbations generated using perturb_texts function.

    span_length (int): The length to which we would span words.
    n_perturbations (int): Number of perturbations to perform.

    Returns: Results on running the source model on the perturbations generated on original and sampled texts.
    """
    load_mask_model()
